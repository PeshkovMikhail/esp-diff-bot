...
XV::XV() {
  ...
  scan_rpm_min = DEFAULT_SCAN_RPM*0.8;
  scan_rpm_max = DEFAULT_SCAN_RPM*1.1;
  pwm_val = 0.5;
...
bool XV::setScanRPM(float rpm) {
  scanRpmPID.setpoint = (rpm <= 0) ? DEFAULT_SCAN_RPM : rpm;
  return true;
}

void XV::setScanRpmPIDSamplePeriod(int sample_period_ms) {
  scanRpmPID.setDt(sample_period_ms);
}
...
void XV::ProcessSpeed() {
  ...
  scanRpmPID.input = float( (scan_rph_high_byte << 8) | scan_rph_low_byte ) / 64.0;
  if (scanRpmPID.input <= 0.1)
    ESP_LOGI("XV_LIB", "Lidar speed %f rpm", scanRpmPID.input);
}
...
*/
uint8_t XV::ProcessDistance(int iQuad) {
  uint8_t dataL, dataM;
...
bool XV::loop() {
  ...
  pwm_val = scanRpmPID.getResultNow();
  if (pwm_val != pwm_last) {

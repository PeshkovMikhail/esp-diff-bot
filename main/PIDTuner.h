...
class PIDtuner {
   public:
    void setParameters(bool newDirection, float newSteady, float newStep, int newWait, float newWindow, uint16_t newPulse, int newPeriod) {
        steady = newSteady;
            ...
            switch (state) {
                ...
                case 1:                                  // ждём стабилизации значения
                    if (millis() - startTime >= wait) {  // ждём стабилизации
                        startTime = millis();
                        ESP_LOGI("STRANNAYA HUETA", "%f %f %f",prevValue,realValue,window);
                        if (abs(prevValue - realValue) < window) {
                            ESP_LOGI("PID", "Stabilized");
                            state = 2;
                    ...
                    if (thisDerivative < 0) {         // значение падает
                        ...
                        if (realValue > steadyValue) {  // проскочили среднюю линию
                            if (trigger) {
                                ...
                                accuracy = 100.0f * std::min(oscTime, prevOscTime) / std::max(oscTime, prevOscTime);
                                startTime = millis();

                                float Ku = abs(4.0 * step / (maxVal - minVal) / 3.1415);
    ...
    void print_parameters() {
        ESP_LOGE("TUNER", "%f %f %f %d", steady, step, window, wait);
    }
